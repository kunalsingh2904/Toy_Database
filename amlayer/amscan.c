...
if (value == NULL)
  {
   ...
   //bcopy(pageBuf + AM_sl , AM_scanTable[scanDesc].nextKey, attrLength);
...
AM_FindNextKey(scanDesc, nextKey)
int scanDesc;/* index scan descriptor */
char *nextKey;
{
    int recId; /* recordId to be returned */
    ...
    { AM_scanTable[scanDesc].status = BUSY;
        ...
        //printf("%s", AM_scanTable[scanDesc].nextvalue);
    }
    ...
    //printf("FCewfvw\n");
    /* copy the recId to be returned */
    //bcopy(pageBuf + AM_scanTable[scanDesc].nextRecIdPtr - header->attrLength, nextIndex,AM_si);
    //char nextKey[header->attrLength];
    bcopy(AM_scanTable[scanDesc].nextvalue, nextKey, 512);
